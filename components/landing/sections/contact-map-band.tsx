...
export function ContactMapBand() {
  ...
  return (
    <MotionItem className="mt-20 border-t border-primary/15">
      <div className="relative min-h-[28rem] overflow-hidden bg-primary">
        <iframe
        ...
        <div className="relative z-10 mx-auto flex min-h-[28rem] max-w-7xl items-center px-4 py-16 sm:px-6 lg:px-8">
          <div className="max-w-lg text-white">
            <p className="eyebrow eyebrow-light">{t.contact.mapKicker}</p>
            <h3 className="font-display mt-6 text-4xl leading-[1.02] sm:text-5xl">{t.contact.mapTitle}</h3>
            <p className="mt-5 max-w-md text-sm leading-relaxed text-white/60">{t.contact.mapSubtitle}</p>
            <p className="mt-8 flex items-center gap-3 font-mono text-[0.62rem] uppercase tracking-[0.16em] text-white/70">
              <MapPin className="size-4 text-accent-bright" />
              {t.contact.officeAddress}
            </p>
            <a href={GOOGLE_MAPS_DIRECTIONS_URL} target="_blank" rel="noopener noreferrer" className="btn-primary mt-8">
              {t.contact.mapCta}
