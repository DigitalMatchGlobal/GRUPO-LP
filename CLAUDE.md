...
- **Process (`#proceso`, light)** — **timeline vertical ligado al scroll**: el “lomo del
  ...
  Imágenes `proceso 1-4.png`: en desktop miniatura a la derecha, en mobile banner full-width
  arriba (`order-first`); en ambos el color se revela al activarse el folio (`lazy`, `sizes` responsivo).
...
2. **Redes sociales**: `SOCIAL_LINKS` vacío en `lib/site.ts`. **Logos clientes**: `CLIENT_LOGOS` vacío.
3. Si se reincorporan Trust/Reviews: cifras y testimonios reales (hoy placeholder en `translations.ts`).
4. Definir qué hacer con las **secciones huérfanas** (ver arriba).

🟢 **Ya resuelto:** favicon + íconos + `og-image` migrados a marca LP (`app/favicon.ico` es el
monograma **blanco sobre transparente** — ojo: puede verse tenue en pestañas claras; alternativa
recomendada = blanco sobre navy). Navegación con URLs limpias sin `#` (`SmoothAnchors`).
